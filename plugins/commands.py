...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        ...
        btn = [[
            InlineKeyboardButton('⚡️ 𝚄𝙿𝙳𝙰𝚃𝙴𝚂 ⚡️', url=UPDATES_LINK),
            InlineKeyboardButton('💡 𝚂𝚄𝙿𝙿𝙾𝚁𝚃 💡', url=SUPPORT_LINK)
        ]]
    ...
    if (len(message.command) != 2) or (len(message.command) == 2 and message.command[1] == 'start'):
        buttons = [[
            InlineKeyboardButton('ℹ️ 𝚄𝙿𝙳𝙰𝚃𝙴𝚂', url=UPDATES_LINK),
            InlineKeyboardButton('🧑‍💻 𝚂𝚄𝙿𝙿𝙾𝚁𝚃', url=SUPPORT_LINK)
        ],[
            InlineKeyboardButton('🔎 𝚂𝙴𝙰𝚁𝙲𝙷 𝙸𝙽𝙻𝙸𝙽𝙴', switch_inline_query_current_chat='')
        ]]
